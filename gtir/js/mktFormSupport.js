    ...
    if (valid === true) {
      if (required) {
        if ((fld.tagName.toUpperCase() == 'INPUT') || (fld.tagName.toUpperCase() == 'TEXTAREA'))  {
          ...
          if (fld.value.replace(/^\s+/g, "").length == 0 || fld.value.indexOf(" *") != -1) {
...
Mkto.clearError = function(fld) {
  var loc = fld.className.indexOf(" mktError"); //fld.parentNode.className.indexOf(" mktError");
  if (loc != -1) {
	  fld.className = fld.className.substr(0, loc);
    var msgContainer = Mkto.getMessage(fld);
	loc = msgContainer.className.indexOf(" showError"); 
	msgContainer.className = msgContainer.className.substr(0, loc);
...
Mkto.setError = function(fld, message) {
  fld.className += " mktError";
  var msgContainer = Mkto.getMessage(fld);
  msgContainer.className += " showError";
...
Mkto.init();
/* Removed from page and placed in this file*/
